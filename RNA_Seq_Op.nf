...
process createTSV{
  ...
  secret 'OPAPI_TOKEN_SECRET'
  input:
    val opapi_base_url from params.opapi_base_url
	  val opapi_verify_https from params.opapi_verify_https
	  val seq_read_set_name from params.seq_read_set_name
	  val tmp_workfile_dir from params.tmp_workfile_dir

	output:
		file("this_tsv_file.tsv") into (
      readInput_to_runFastQC,
      readInput_to_runSTAR1pass,
      readInput_to_runSTAR2pass,
      generatedTSVInfile,
      resultsPostingTSVFile
    )
    script:
...
process runSTAR1pass {
  ...
  set indivID, sampleID, libraryID, rgID, platform_unit, platform,
    ...
    from readInput_to_runSTAR1pass.splitCsv(sep: '\t', header: true)
...
process runSTAR2pass {
  ...
  set indivID, sampleID, libraryID, rgID, platform_unit, platform,
    platform_model, run_date, center, R1, R2 \
    from readInput_to_runSTAR2pass.splitCsv(sep: '\t', header: true)
...
process runFastQC {
  ...
  set indivID, sampleID, libraryID, rgID, platform_unit, platform,
    platform_model, run_date, center, R1, R2 \
    from readInput_to_runFastQC.splitCsv(sep: '\t', header: true)
...
process createSE {
  ...
  file generated_infile from generatedTSVInfile
  output:
  ...
  output[["inputfile"]] <- read.delim(
    "${generated_infile}", stringsAsFactors=FALSE
  )
...
process postResults {
  tag "Post Files to Operend"
  publishDir "${params.output_dir}/Operend/posts"
  secret 'OPAPI_TOKEN_SECRET'
  
  input:
    file theSeOut from runCreateSEOutput
    val output_dir from params.output_dir
    file input_tsv_file from resultsPostingTSVFile
    val job_type_name from params.job_type_name
    val job_run_label from params.prefix
    val seq_read_set from params.seq_read_set_name
    val opapi_base_url from params.opapi_base_url
    val opapi_verify_https from params.opapi_verify_https
    val opapi_storage_location from params.opapi_storage_location
  
  
  script:
    template 'post_nf_output.py' 
