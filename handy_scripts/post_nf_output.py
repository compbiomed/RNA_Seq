...
def post_results(project_name,study_name,seq_read_set_name,input_tsv_ID):
    ...
    if not TEST_RUN:
        ...
        #jr_json =json.dumps(jr, indent = 4)
        # print(jr_json)
        ...
        for ebods in coll_ent_bodies:
            for e in ebods:
                ...
                #e['study'] = study_name
                b['seq_read_set_name'] = seq_read_set_name
                e['job_run_label'] = str(jrun.id)
                coll_ent = Entity(entity_class='RNASeqRes_Collective',
                                values=e)
                coll_ent.save()
...
if __name__ == "__main__":
    ...
    print(os.getcwd())
    ApiBacked.configure_from_file('/usr2/collab/dulrich/development/RNA_Seq/config.ini')
    INPUT_FILE = "/usr2/collab/dulrich/development/RNA_Seq/this_tsv_file.tsv"
    OUT_DIR = "/rprojectnb/pulmseq/daupipe/OUTPUT/"

    
    parser = argparse.ArgumentParser()
    parser.add_argument("-r","--results_directory", type=str,
                        help="the directory containing the results or publish directory of a pipeline run",
                        required=True)

    parser.add_argument("-j","--job_type_name", type=str,
                        help="the directory containing the results of a job run",
                        required=True)
    ...
    input_wf_id = -999
    if not TEST_RUN:
        input_wf_id = WorkFile.post_from_file(args.input_file).systemId

    #not using project or study at the moment
    post_results(None,None,args.seq_read_set_name,input_wf_id)
    sys.exit()

    """
    We can assume that the input tsv file has not been posted
    """
